...
import (
	...
	"ecommerce-order-product/events"
...
func StartInventoryWorker(ctx context.Context, db *gorm.DB, orderRepository repositories.OrderRepository, productRepository repositories.ProductRepository) error {
...
func processOrder(db *gorm.DB, p *events.OrderPlacedPayload, orderRepository repositories.OrderRepository) error {
...
func publishOrderResult(order *entities.Order, status, reason string) {
	ch, err := config.NewChannel()
	if err != nil {
